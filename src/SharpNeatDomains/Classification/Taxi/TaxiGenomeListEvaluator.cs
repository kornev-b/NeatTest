...
namespace SharpNeat.Domains.Classification.Taxi
{
    ...
    /// </summary>
    public class TaxiGenomeListEvaluator<TGenome> : IGenomeListEvaluator<TGenome>
        where TGenome : class, IGenome<TGenome>
    {
        readonly IGenomeDecoder<TGenome, IBlackBox> _genomeDecoder;
        readonly TaxiBlackBoxEvaluator _phenomeEvaluator;
        ...
        /// </summary>
        public TaxiGenomeListEvaluator(IGenomeDecoder<TGenome, IBlackBox> genomeDecoder,
                                           TaxiBlackBoxEvaluator phenomeEvaluator,
        ...
        {
            if (!OverfittingParams.interleaved)
            {
                EvaluateSubsample(genomeList, OverfittingParams.subsample);
                return;
            }
            if (flag)
            {
                EvaluateSubsample(genomeList, OverfittingParams.interleavedStartSubsample);
                flag = false;
                return;
            }

            EvaluateSubsample(genomeList, OverfittingParams.interleavedCrossSubsample);
            flag = true;
        }
