...
{
    public class KobyNeatEvolutionAlgorithm:NeatEvolutionAlgorithm<NeatGenome>
    {
        private double bestFitness = 0;
        private const double FITNESS_THRESHOLD = 1.62;
        private readonly KobyDataProvider _dataProvider;
        private List<int> generationTrack = new List<int>(new int[]{0, 50, 100, 500});
        XmlWriterSettings _xwSettings = new XmlWriterSettings();
 
        public KobyNeatEvolutionAlgorithm(KobyDataProvider dataProvider, NeatEvolutionAlgorithmParameters eaParams, ISpeciationStrategy<NeatGenome> speciationStrategy, IComplexityRegulationStrategy complexityRegulationStrategy) : base(eaParams, speciationStrategy, complexityRegulationStrategy)
        {
        ...
        protected override void UpdateBestGenome()
        {
            ...
            eval.logloss = 1/eval.logloss;
            _currentBestGenome.EvaluationInfo.SetEvalFitness(eval.logloss);
            if (eval.logloss > FITNESS_THRESHOLD && eval.logloss > bestFitness)
            {
                bestFitness = eval.logloss;
                saveChampion();
            }
